...
// 输入编码为HEX
static int encodeHex(int argc, char *argv[])
{
    if (argc <= 3)
    {
        printError("Parameter error.");
        printUsage(argv[0], "encode hex \"0x** 0x** ...\"");
        return -1;
    }
...
// 输入编码为字符串
static int encodeString(int argc, char *argv[])
{
    if (argc <= 3)
    {
        printError("Parameter error.");
        printUsage(argv[0], "encode str xxx string");
        return -1;
    }
...
static int decode(int argc, char *argv[])
{
    ...
    {
        printError("Parameter error.");
        printUsage(argv[0], "decode xxx string");
        return -1;
    }
...
static int _cmd(int argc, char *argv[])
{
    ...
    {
        printf("Error: Invalid command.\r\nUsage:\r\n");
        printf("%s encode hex/str\r\n", argv[0]);
        printf("%s decode string\r\n", argv[0]);
        return -1;
    }
    ...
    if (strcmp(argv[1], "encode") == 0)
    {
        ...
        {
            return encodeHex(argc, argv);
        }
        else if (strcmp(argv[2], "str") == 0)
        {
            return encodeString(argc, argv);
        }
        else
        {
