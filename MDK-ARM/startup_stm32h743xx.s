...
; Vector Table Mapped to Address 0 at Reset
                AREA    RESET, DATA, READONLY   ; 声明名为"RESET"的DATA数据段，权限为"READONLY"只读
                EXPORT  __Vectors               ; 导出标号 __Vectors, 理解为向量表首地址，分散加载描述文件用到
                EXPORT  __Vectors_End           ; IAR 编译器，则使用的是 GLOBAL 指令
...
__Vectors_Size  EQU  __Vectors_End - __Vectors
                ...
                AREA    |.text|, CODE, READONLY   ; 声明".text"的代码段，权限为"READONLY"只读

; Reset handler
Reset_Handler    PROC               ; 定义子程序，与 ENDP 成对使用
                 EXPORT  Reset_Handler                    [WEAK]
        IMPORT  SystemInit
        IMPORT  __main

                 LDR     R0, =SystemInit    ; 存储器中加载字到一个寄存器中
                 BLX     R0                 ; 跳转至reg给出的地址,并根据reg的LSB位切换处理器，转移前的下条指令地址保存到LR(R14)
                 LDR     R0, =__main
                 BX      R0                 ; 跳转到由寄存器reg给出的地址
                 ENDP
